...
class IndexViewTestCase(SolosBaseTestCase):
    ...
    @patch('solos.views.get_artist_tracks_from_musicbrainz')
    def test_index_view_returns_external_tracks(self, mock_get_tracks_from_mb):
        """
        Test that the index view will return artists from the MusicBrainz API if none are returned from our database
        """
        response = self.client.get('/', {
            'instrument': 'Bass',
            'artist': 'Jaco Pastorius' # not in the database
        })

        self.assertEqual(mock_get_tracks_from_mb.method_calls, [call.delay(artist='Jaco Pastorius')])
